...
const createTextureAtlas = (textures, isText = false) => {
  const atlasSize = Math.max(1, Math.ceil(Math.sqrt(Math.max(1, textures.length))));
...
const onPointerUp = (event) => {
  ...
  if (isClick && Date.now() - clickStartTime < 200) {
    ...
    endY = event.clientY || event.changedTouches?.[0]?.clientY;
  }
  ...
  if (endX !== undefined && endY !== undefined) {
    ...
    const texIndex = Math.floor(((cellX + cellY * 3.0) % projects.length + projects.length) % projects.length);
...
const init = async () => {
  ...
  const uniforms = {
    ...
    uTextureCount: { value: Math.max(1, projects.length) },
  ...
  console.log("Mesh:", plane);
};

// Export init function so it can be called from HTML
window.init = init;

// Only auto-init if splash screen doesn't exist
